...
let of_string_exn message =
  ...
  let single_symbol json =
    json |> member "symbol" |> to_string |> Symbol.of_string_exn
  in
  let symbol_list json ~f =
    json |> member "symbols" |> to_list |> List.map ~f
  in
  let symbol_set json =
  ...
  in
  let order_id json =
    json |> member "order_id" |> to_int |> Order_id.of_int_exn
  in
  ...
  | "hello" ->
    let symbols =
      symbol_list json ~f:(fun json ->
        let position =
          json |> member "position" |> to_int |> Position.of_int_exn
        in
  ...
  | "book" ->
    let parse_one_side json =
      ...
      |> List.map ~f:(fun level ->
        ...
        | _ ->
          raise_s [%message "Ill formatted level in book message" message])
    in
  ...
  | "trade" ->
    Trade
      { symbol = single_symbol json; price = price json; size = size json }
...
let%test_module "Message deserialization tests" =
  (module struct
    ...
    let%expect_test "TRADE" =
      let message =
        {|{"type":"trade","symbol":"MS","price":4106,"size":1}|}
      in
      Core.printf !"%{sexp: t}\n%!" (of_string_exn message);
    ...
    let%expect_test "REJECT" =
      let message =
        {|{"type":"reject","order_id":42,"error":"LIMIT:PRICE_BAND"}|}
      in
      Core.printf !"%{sexp: t}\n%!" (of_string_exn message);
    ...
    let%expect_test "FILL" =
      ...
      Core.printf !"%{sexp: t}\n%!" (of_string_exn message);
      [%expect
        {| (Fill ((order_id 3) (symbol BOND) (dir Sell) (price 999) (size 10))) |}]
    ;;
